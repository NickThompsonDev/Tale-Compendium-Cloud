...
output "ingress_ip" {
  description = "The external IP of the ingress controller"
  value       = try(kubernetes_ingress.webapp-api-ingress.status[0].load_balancer[0].ingress[0].ip, "Pending IP allocation")
}
